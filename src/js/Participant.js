    ...
    document.querySelectorAll(".calendar-header__option").forEach((option) => {
      option.addEventListener("click", function () {
        if (!option.classList.contains("selected")) {
          if (option.parentNode.querySelector(".calendar-header__option.selected"))
            option.parentNode.querySelector(".calendar-header__option.selected").classList.remove("selected")
            ...
            this.draggable =
            el.authorisedBtnDOM.innerText === "Anna" ? el.authorisedBtnDOM.innerText === "Anna" : false
