...
ee.on("delete-event", (id) =>
  request.deletEventData(id).then(() => events.getEvents())
);
ee.on("put-event", (event, id) => {
  request.putEventData(event, id).then(() => events.getEvents());
});
...
class Admin extends User {
  constructor(name) {
    super(name);
    this.name = name;
    this.calendarMeetingWrapper = document.querySelectorAll(
      ".calendar__meeting-wrapper"
    );
  ...
  addDraggableAtr(ev) {
    if (ev.target.closest(".calendar__meeting-wrapper"))
      this.calendarMeetingWrapper.forEach((elem) =>
  ...
  dragStart(ev) {
    ...
    if (!ev.target.closest(".calendar-header__select"))
      document
        .querySelector(".calendar-header__select")
  ...
  drop(ev) {
      ...
      meetings.some(
        (meeting) =>
          JSON.parse(meeting.data)
            .day.substring(0, 2)
            .concat("-", JSON.parse(meeting.data).time.substring(0, 2))
            .toLowerCase() === target.id
      )
    ) {
      this.warnning.closeWarning();
      el.warningMessage.children[0].children[1].innerHTML =
        "Failed to move the event. Time slot is already taken!";
    ...
    this.draggedMeeting = JSON.parse(
      events
        .getEvents()
        .filter(
          (meeting) =>
    ...
    this.draggedMeeting.day = this.daysOfWeek.filter(
      (day) => day.substring(0, 2).toLowerCase() === target.id.substring(0, 2)
    )[0];
    this.draggedMeeting.time = this.hours.filter(
      (time) => time.substring(0, 2) === target.id.substring(3, 5)
    )[0];
