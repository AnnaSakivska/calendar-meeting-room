...
export default class Authorisation {
  ...
  insertAllMeetings() {
    this.displayedMeeting = new DisplayedMeeting(
      events.getEvents(),
      adminIsLoggedIn
    );
    if (!events.getEvents()) {
      this.warnning = new Warnning(
        el.messageSuccessful,
        "Your calendar is empty! No meetings have been planned!"
      );
...
// Authorization
el.authorizationBtnDOM.addEventListener("click", () =>
  authorisation.authorise()
);

// Reauthorise
el.authorisedBtnDOM.addEventListener("click", () =>
  el.authorisePopupDom.classList.remove("d-none")
);

el.clendarMeetingSpotDom.forEach((elem) =>
  elem.addEventListener("mousedown", (ev) => user.addDraggableAtr(ev))
);
...
el.clendarMeetingSpotDom.forEach((elem) =>
  elem.addEventListener("drop", (ev) => user.drop(ev))
);
el.clendarMeetingSpotDom.forEach((elem) =>
  elem.addEventListener("dragend", (ev) => user.dragEnd(ev))
);
el.clendarMeetingSpotDom.forEach((elem) =>
  elem.addEventListener("dragover", (ev) => user.dragOver(ev))
);

// Delete meeting
el.clendarMeetingSpotDom.forEach((elem) =>
  elem.addEventListener("click", (ev) => user.showDeletePop(ev))
);
el.deleteOkBtn.addEventListener("click", () => user.deleteMeeting());
el.deleteNotBtn.addEventListener("click", () =>
