...
ee.on('post-event', (event, method) => { request.postEventData(event).then(() => method()) })
...
class NewEventWindow {
  ...
  creatNewEvent(members) {
    ...
    if (events.getEvents() && events.getEvents().some(meeting => JSON.parse(meeting.data).time === planningMeeting.time && JSON.parse(meeting.data).day === planningMeeting.day)) {
